...
class PPO:
    def __init__(self, state_dim, action_dim, 
                 ...
                 sim_env=None, total_its=1000, eval_freq=100, save_freq=100, num_worker=8, env_config=None):
    ...
    def worker_collect_data(self, worker_id, actor_state_dict, critic_state_dict, iterations, env_config, obs_history_length):
        """
        在子进程中采样数据：
        - actor_state_dict, critic_state_dict：主进程传入的最新模型参数
        - iterations：本进程采样步数
        - env_config：用于构造环境的参数，例如 (n_vehs, sectors, n_vehs_in_state)
        - obs_history_length：历史观测长度（例如3）
        返回一个列表，每个元素为 (state, action, log_prob, value, reward, done)
        """
        import torch
        import numpy as np
        from simulator import Simulator  # 确保在子进程中导入

        n_vehs, sectors, n_vehs_in_state = env_config
        state_dim = (n_vehs*2+1+4)*4  # 与主代码中计算一致
        action_dim = 2

        # 创建环境实例（每个进程独立）
        sim_env = Simulator(n_vehs, sectors, n_vehs_in_state)

        # 创建并加载 actor 与 critic 网络
        ...
        data = []
        obs = sim_env.reset()
        # 初始化观测历史，长度 obs_history_length
        obs_history = [obs.copy() for _ in range(obs_history_length)]
        concatenated_obs = np.concatenate([obs] + obs_history)
        
        for t in range(iterations):
            # 构造输入并采样动作
            input_tensor = torch.tensor(concatenated_obs, dtype=torch.float32).unsqueeze(0)
            with torch.no_grad():
                probabilities = actor(input_tensor)
                value = critic(input_tensor).item()
                dist = torch.distributions.Categorical(probabilities)
                action = dist.sample().item()
                log_prob = dist.log_prob(torch.tensor(action)).item()
            
            next_obs, reward, done = sim_env.step(action)
            # 存储当前转移
            data.append((concatenated_obs, action, log_prob, value, reward, done))
    ...
    def train(self, num_episodes=10000):
        ...
        import multiprocessing as mp
        obs_history_length = 3  # 与原代码保持一致
        
        for episode in range(num_episodes):
            print(f'Episode: {episode}')
            # 计算每个进程采样步数
            iterations_per_worker = 5000
            # 准备每个子进程的参数：传入当前最新的 actor 和 critic 参数
            worker_args = []
            for worker_id in range(self.num_worker):
                worker_args.append((worker_id, 
            ...
            with mp.Pool(processes=self.num_worker) as pool:
                results = pool.starmap(self.worker_collect_data, worker_args)
            # 将所有进程采样的数据存入 memory
            for worker_data in results:
                for transition in worker_data:
                    self.memory.store_memory(*transition)
            # 更新模型
            ep_actor_losses, ep_critic_losses, ep_entropy = self.learn()
            # 这里简单统计一个 epoch 内动作 1 的比例（非贪心动作）
            non_greedy_count = sum([1 for (_, action, _, _, _, _) in self.memory.states if action == 1])
